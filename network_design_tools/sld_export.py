...
def createSLD(iface, bdry_lyr):
    ...
    for node in nodes_lyr.getSelectedFeatures():
        ...
        node_label = '<<b>{}<br />{}P<br />{}</b>>'.format(node_id, node[node_flds['premises']], \
            '<br />'.join(wrap(node[node_flds['address']], width=25)))
        with g.subgraph(name='cluster_{}'.format(node_id)) as c:
            ...
            c.node('{}_L_html'.format(node_id), label=node_label, shape='plain')

    # Add secondary nodes
    processing.run("qgis:selectbylocation", { 'INPUT' : nodes_lyr, 'INTERSECT' : bdry_sel_lyr, 'METHOD' : 0, 'PREDICATE' : [0] })
    nodes_lyr.selectByExpression('\"{}\" = 2'.format(node_flds['use']), QgsVectorLayer.IntersectSelection)
    for node in nodes_lyr.getSelectedFeatures():
        node_id = node[node_flds['id']]
        node_label = '<<b>{}<br />{}x1:8 ({}P)<br />{}</b>>'.format(node_id, node[node_flds['splitters_1_8']], node[node_flds['premises']], \
            '<br />'.join(wrap(node[node_flds['address']], width=25)))
        with g.subgraph(name='cluster_{}'.format(node_id)) as c:
            c.attr(style='invis')
            c.node(node_id, label='UGSN', shape='square', width='1', height='1', fixedsize='shape')
            c.node('{}_F_html'.format(node_id).format(node_id), label='<<b>{}F</b>>'.format(node[node_flds['splitters_1_8']]), shape='plain', fontcolor='Red')
            c.node('{}_L_html'.format(node_id), label=node_label, shape='plain')

    processing.run("qgis:selectbylocation", { 'INPUT' : nodes_lyr, 'INTERSECT' : bdry_sel_lyr, 'METHOD' : 0, 'PREDICATE' : [0] })
    nodes_lyr.selectByExpression('\"{}\" = 3'.format(node_flds['use']), QgsVectorLayer.IntersectSelection)
    for node in nodes_lyr.getSelectedFeatures():
        node_id = node[node_flds['id']]
        node_label = '<<b>{}<br />{}x1:8 ({}P)<br />{}</b>>'.format(node_id, node[node_flds['splitters_1_8']], node[node_flds['premises']], \
            '<br />'.join(wrap(node[node_flds['address']], width=25)))
        with g.subgraph(name='cluster_{}'.format(node_id)) as c:
            ...
            c.node('{}_F_html'.format(node_id), label='<<b>{}F</b>>'.format(node[node_flds['splitters_1_8']]), shape='plain', fontcolor='Red')
            c.node('{}_L_html'.format(node_id), label=node_label, shape='plain')

    processing.run("qgis:selectbylocation", { 'INPUT' : nodes_lyr, 'INTERSECT' : bdry_sel_lyr, 'METHOD' : 0, 'PREDICATE' : [0] })
    nodes_lyr.selectByExpression('\"{}\" = 6'.format(node_flds['use']), QgsVectorLayer.IntersectSelection)
    for node in nodes_lyr.getSelectedFeatures():
        node_id = node[node_flds['id']]
        node_label = '<<b>{}<br />{}x1:8 ({}P)<br />{}</b>>'.format(node_id, node[node_flds['splitters_1_8']], node[node_flds['premises']], \
            '<br />'.join(wrap(node[node_flds['address']], width=25)))
        with g.subgraph(name='cluster_{}'.format(node_id)) as c:
            c.attr(style='invis')
            c.node(node_id, label='MSN', shape='triangle', width='1.1', height='1.1', fixedsize='shape')
            c.node('{}_F_html'.format(node_id), label='<<b>{}F</b>>'.format(node[node_flds['splitters_1_8']]), shape='plain', fontcolor='Red')
            c.node('{}_L_html'.format(node_id), label=node_label, shape='plain')

    # Add tertiary nodes
    processing.run("qgis:selectbylocation", { 'INPUT' : nodes_lyr, 'INTERSECT' : bdry_sel_lyr, 'METHOD' : 0, 'PREDICATE' : [0] })
    nodes_lyr.selectByExpression('\"{}\" = 5'.format(node_flds['use']), QgsVectorLayer.IntersectSelection)
    for node in nodes_lyr.getSelectedFeatures():
        node_id = node[node_flds['id']]
        node_label = '<<b>{}<br />{}x1:8 ({}P)<br />{}</b>>'.format(node_id, node[node_flds['splitters_1_8']], node[node_flds['premises']], \
            '<br />'.join(wrap(node[node_flds['address']], width=25)))
        with g.subgraph(name='cluster_{}'.format(node_id)) as c:
            c.attr(style='invis')
            c.node(node_id, label='UGCE', shape='square', width='0.7', height='0.7', fixedsize='shape')
            c.node('{}_F_html'.format(node_id), label='<<b>{}F</b>>'.format(node[node_flds['splitters_1_8']]), shape='plain', fontcolor='Red')
            c.node('{}_L_html'.format(node_id), label=node_label, shape='plain')


    processing.run("qgis:selectbylocation", { 'INPUT' : nodes_lyr, 'INTERSECT' : bdry_sel_lyr, 'METHOD' : 0, 'PREDICATE' : [0] })
    nodes_lyr.selectByExpression('\"{}\" = 4'.format(node_flds['use']), QgsVectorLayer.IntersectSelection)
    for node in nodes_lyr.getSelectedFeatures():
        node_id = node[node_flds['id']]
        node_label = '<<b>{}<br />{}x1:8 ({}P)<br />{}</b>>'.format(node_id, node[node_flds['splitters_1_8']], node[node_flds['premises']], \
            '<br />'.join(wrap(node[node_flds['address']], width=25)))
        with g.subgraph(name='cluster_{}'.format(node_id)) as c:
            c.attr(style='invis')
            c.node(node_id, label='PMCE', shape='circle', width='0.7', height='0.7', fixedsize='shape')
            c.node('{}_F_html'.format(node_id), label='<<b>{}F</b>>'.format(node[node_flds['splitters_1_8']]), shape='plain', fontcolor='Red')
            c.node('{}_L_html'.format(node_id), label=node_label, shape='plain')


    processing.run("qgis:selectbylocation", { 'INPUT' : nodes_lyr, 'INTERSECT' : bdry_sel_lyr, 'METHOD' : 0, 'PREDICATE' : [0] })
    nodes_lyr.selectByExpression('\"{}\" = 7'.format(node_flds['use']), QgsVectorLayer.IntersectSelection)
    for node in nodes_lyr.getSelectedFeatures():
        node_id = node[node_flds['id']]
        node_label = '<<b>{}<br />{}x1:8 ({}P)<br />{}</b>>'.format(node_id, node[node_flds['splitters_1_8']], node[node_flds['premises']], \
            '<br />'.join(wrap(node[node_flds['address']], width=25)))
        with g.subgraph(name='cluster_{}'.format(node_id)) as c:
            c.attr(style='invis')
            c.node(node_id, label='MCE', shape='triangle', width='0.8', height='0.8', fixedsize='shape')
            c.node('{}_F_html'.format(node_id), label='<<b>{}F</b>>'.format(node[node_flds['splitters_1_8']]), shape='plain', fontcolor='Red')
            c.node('{}_L_html'.format(node_id), label=node_label, shape='plain')
    ...
    for joint in joints_lyr.getSelectedFeatures():
        ...
        joint_label = '<<b>{}<br />{}<br />{}</b>>'.format(joint_id, joint[joint_flds['size']], '<br />'.join(wrap(joint[joint_flds['address']], width=25)))
        with g.subgraph(name='cluster_{}'.format(joint[joint_flds['id']])) as c:
            ...
            c.node('{}_L_html'.format(joint_id), label=joint_label, shape='plain')
